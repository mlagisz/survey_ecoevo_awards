...
summary(mod_BR)
#calculate % difference between genders at the intercept (year 2005):
plogis(summary(mod_BR)$coef[1])*100 # 25% fewer female names
summary(mod_BR)$coef[2,] #signif. increasing proportion of females across decades
```
...
#mod <- glmer(cbind(female, male) ~ sdecade + (1| Award), family = "binomial", data = dat) # does not run:boundary (singular) fit: see help('isSingular')  Probably due  to having too few decades.
mod_BP <- glmer(cbind(female, male) ~ (1| Award), family = "binomial", data = dat_BP) 
summary(mod_BP)

#calculate % difference between genders at the intercept:
plogis(summary(mod_BP)$coef[1])*100 # 44% fewer female names
```
...
#model_BR <- glmer(Gender ~ (1|Award_name), family = "binomial", data = GEOdata_BR) #without year as a predictor - not working

summary(model_BR)

#calculate % difference between genders at the intercept:
plogis(summary(model_BR)$coef[1])*100 # 72% fewer female names - similar pattern to the analyses across decades
```
...
model_BP <- glmer(Gender ~ (1|Award_name), family = "binomial", data = GEOdata_BP) #without year as a predictor

summary(model_BP)

#calculate % difference between genders at the intercept:
plogis(summary(model_BP)$coef[1])*100 # 58% fewer female names- similar pattern to the analyses across decades
```
